  ...
  const body = (
    <>
      ...
      <div className={isSmall ? "space-y-0.5 p-2" : "space-y-1 p-3"}>
        <h3 className={isSmall ? "line-clamp-2 text-sm font-medium leading-tight lg:text-xs" : "line-clamp-2 text-sm font-medium leading-tight"}>{title}</h3>
        <p className={isSmall ? "line-clamp-1 text-xs text-muted-foreground lg:text-[10px]" : "line-clamp-1 text-xs text-muted-foreground"}>{authors}</p>
        <div className="flex flex-wrap gap-1">
          {formats.map((f) => (
            <Badge key={f} variant="secondary" className={isSmall ? "px-1 py-0 text-[10px] lg:text-[8px]" : "px-1.5 py-0 text-[10px]"}>
              {f}
            </Badge>
          ))}
          {series && (
            <Badge data-testid="series-badge" title={series} variant="outline" className={isSmall ? "truncate px-1 py-0 text-[10px] lg:text-[8px]" : "truncate px-1.5 py-0 text-[10px]"}>
              {series}
            </Badge>
          )}
          {enrichmentStatus === "STUB" && scanActive && (
            <Badge variant="outline" className={isSmall ? "animate-pulse px-1 py-0 text-[10px] lg:text-[8px]" : "animate-pulse px-1.5 py-0 text-[10px]"}>
              Processing&hellip;
            </Badge>
          )}
        </div>
