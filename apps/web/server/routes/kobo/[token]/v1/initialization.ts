...
export interface InitializationHandlerDeps {
  auth: KoboAuthDeps;
  getBaseUrl: () => string;
...
/**
 * The full native Kobo resource dictionary.
 * This is the same dict calibre-web starts with before overriding specific keys.
...
export function createInitializationHandler(deps: InitializationHandlerDeps) {
  return async (event: H3Event) => {
    ...
    const prefix = `${baseUrl}/kobo/${device.authToken}`;

    const resources = nativeKoboResources();

    // Override only the keys that need to point to our server
    resources.image_host = baseUrl;
    resources.image_url_quality_template = `${prefix}/{ImageId}/{Width}/{Height}/{Quality}/{IsGreyscale}/image.jpg`;
    resources.image_url_template = `${prefix}/{ImageId}/{Width}/{Height}/false/image.jpg`;
    resources.library_sync = `${prefix}/v1/library/sync`;
    resources.oauth_host = `${prefix}/oauth`;

    // Matches calibre-web: hardcoded empty JSON object as base64
    deps.setResponseHeader(event, "x-kobo-apitoken", "e30=");
