...
describe("_authenticated route", () => {
  ...
  it("beforeLoad redirects when no user", async () => {
    ...
    (getCurrentUserServerFn as any).mockResolvedValue(null);

    const { Route } = await import("./_authenticated");

    await expect(
      Route.beforeLoad({ context: {} } as any)
  ...
  it("beforeLoad returns user when authenticated", async () => {
    ...
    (getCurrentUserServerFn as any).mockResolvedValue(user);

    const { Route } = await import("./_authenticated");

    const result = await Route.beforeLoad({ context: {} } as any);
